...
MCP (Model Context Protocol) is a next-generation protocol recommended for IoT control. It uses standard JSON-RPC 2.0 format to discover and invoke "tools" between the backend and devices, enabling flexible device control.

## Typical Usage Flow

1. After device startup, establish a connection with the backend through a base protocol (such as WebSocket/MQTT).
2. The backend initializes the session through the MCP protocol's `initialize` method.
3. The backend retrieves all tools (capabilities) and parameter descriptions supported by the device through `tools/list`.
4. The backend invokes specific tools through `tools/call` to control the device.

For detailed protocol format and interactions, see [`mcp-protocol.md`](./mcp-protocol.md).

## Device-Side Tool Registration Method

Devices register callable "tools" through the `McpServer::AddTool` method. The common function signature is as follows:
...
void AddTool(
    ...
    std::function<ReturnValue(const PropertyList&)> callback // Callback implementation when the tool is invoked
);
```
...
- properties: Parameter list, supports boolean, integer, and string types, with optional range and default value specifications.
- callback: Actual execution logic when a call request is received, return value can be bool/int/string.

## Typical Registration Example (ESP-Hi Example)
